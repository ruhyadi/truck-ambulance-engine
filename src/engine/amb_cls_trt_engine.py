...
if __name__ == "__main__":
    ...
    engine = AmbulanceClsTrtEngine(
        engine_path="tmp/models/mobilenetv3_large_100_Opset16.plan",
        max_batch_size=1,
        categories=[str(i) for i in range(1000)],
    )
    engine.setup()

    img = cv2.imread("tmp/car.png")
    imgs = [img] * 1

    result = engine.predict(imgs=imgs, conf=0.25)
